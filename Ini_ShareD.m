...
%-----------------------------------------------------------------------
	...
	num_class_fixed = size_col/4;
	column_now  = 0;

	while(SharedD_nClass < num_class_fixed && column_now<=size_col)
		column_now = column_now + 1;
		fprintf(['column_now = ' num2str(column_now) '\n']);
		Shared_class_exist = zeros(1, nClass);		%to check if the class already exist in D0
		...
		%delete columns which are also in shared dictionary from there original dictionary 
        for i= 1:nClass
	...
	for i = 1:nClass
		...
		size(temp_HeadDict)
		temp_HeadDict = temp_HeadDict(:,1:900);					%pick the former 80

...
if (size(Data,2) < param.K)  
   ...
   %?•Â??∞KÂ§ß‰?‰ø°Âè∑?Ñ‰∏™???ôÂ??∞ÊçÆ?Ü‰?‰∏∫Â??∏È?  

elseif (strcmp(param.InitializationMethod,'DataElements'))  

   Dictionary(:,1:param.K-param.preserveDCAtom) = Data(:,1:param.K-param.preserveDCAtom);%Â∞ÜÊï∞?ÆÈ????∞param.K-param.preserveDCAtom?∞ÊçÆ‰Ωú‰∏∫Â≠óÂÖ∏?? 

elseif (strcmp(param.InitializationMethod,'GivenMatrix'))  

   Dictionary(:,1:param.K-param.preserveDCAtom) = param.initialDictionary(:,1:param.K-param.preserveDCAtom);%Â∞ÜinitialDictionary???∞param.K-param.preserveDCAtom?ó‰?‰∏∫Â??∏È?  
...
if (param.preserveDCAtom)  
   ...
   tmpMat = FixedDictionaryElement \ Dictionary;%Ê±Çminimal normÔºàFixedDictionaryElement?Dictionary-dictionaryÔº? 
...
Dictionary = Dictionary*diag(1./sqrt(sum(Dictionary.*Dictionary)));%ÂΩí‰??? 

Dictionary = Dictionary.*repmat(sign(Dictionary(1,:)),size(Dictionary,1),1); % multiply in the sign of the first element.%Â≠óÂÖ∏?Ü‰∏≠?ÑÊ?‰∏™Â?Á¥†Á??ñ‰∏∫Ê≠?ï∞  
...
for iterNum = 1:param.numIteration 
   ...
   if (param.errorFlag==0)%?∫Â?Ë°®ËææÁ≥ªÊï∞?Ñ‰∏™?? 
       ...
       CoefMatrix = OMPerr([FixedDictionaryElement,Dictionary],Data, param.errorGoal);%ËÆæÂ??ÅËÆ∏?ÑËØØÂ∑? 
   ...
   rPerm = randperm(size(Dictionary,2));%?üÊ?‰∏?∏™1?∞size(Dictionary,2)?ÑÈ??∫Á??ëÈ?  
...
...
       ...
       Dictionary(:,j) = betterDictionaryElement;%?¥Êñ∞Â≠óÂÖ∏?? 
       ...
       if (param.preserveDCAtom)  
           ...
           Dictionary(:,j) = Dictionary(:,j)./sqrt(Dictionary(:,j)'*Dictionary(:,j));%ÂΩí‰??? 
...
function [betterDictionaryElement,CoefMatrix,NewVectorAdded] = I_findBetterDictionaryElement(Data,Dictionary,j,CoefMatrix,numCoefUsed)%CoefMatrix‰∏∫Â??∏Ê?ÁªàÁ?Á≥ªÊï∞  
...
relevantDataIndices = find(CoefMatrix(j,:)); % ?ûÈõ∂?ÉÂú®Á¨¨jË°åÁ?Á≥ªÊï∞?©Èòµ‰∏≠Á?‰ΩçÁΩÆthe data indices that uses the j'th dictionary element.  

if (length(relevantDataIndices)<1) %(length(relevantDataIndices)==0)Â¶ÇÊ?Á≥ªÊï∞?©Èòµ?ÑÁ¨¨j?óÂÖ®‰∏∫Èõ∂  

   ErrorMat = Data-Dictionary*CoefMatrix;%?®Â∑≤?âÁ?Â≠óÂÖ∏?Ü‰??åÁ≥ª?∞‰?ÂØπdataÈ°πÁ?‰º∞ËÆ°ËØØÂ∑Æ  

   ErrorNormVec = sum(ErrorMat.^2);%ÂØπËØØ Â∑ÆÊ?È°πÂπ≥?? 

   [d,i] = max(ErrorNormVec);%d‰∏∫Ê??âÂ?‰∏≠Ê?Â§ßÈ°πÔºåi‰∏∫ÂÖ∂Á¨¨Â??? 

   betterDictionaryElement = Data(:,i);%ErrorMat(:,i); %?∞ÊçÆÈ°πÁ?i?óË?ÁªôbetterDictionaryElement  

   betterDictionaryElement = betterDictionaryElement./sqrt(betterDictionaryElement'*betterDictionaryElement);%ÂΩí‰??ñbetterDictionaryElement  

   betterDictionaryElement = betterDictionaryElement.*sign(betterDictionaryElement(1));%Â∞ÜbetterDictionaryElement‰∏≠Ë??ÑÂ?Á¥†Â?‰∏∫Ê≠£?? 

   CoefMatrix(j,:) = 0;%Â∞ÜÁ≥ª?∞Áü©?µÁ?Á¨¨jË°åË??º‰∏∫0  
...
tmpCoefMatrix = CoefMatrix(:,relevantDataIndices); %Â∞ÜÁ≥ª?∞Áü©?µÁ?Á¨¨jË°åÁ??ûÈõ∂È°πÊ??®Á??óË?ÁªôtmpCoefMatrix  

tmpCoefMatrix(j,:) = 0;% the coeffitients of the element we now improve are not relevant.Â∞ÜtmpCoefMatrixÁ¨¨jË°åË?0  

errors =(Data(:,relevantDataIndices) - Dictionary*tmpCoefMatrix); %?®Èô§?ªÂ??∏‰∏≠Á¨¨j‰∏™Á??ÉÁ??éÊï∞?ÆÈ?‰∏éÈ?ÊµãÊï∞?Æ‰??¥Á?ËØØÂ∑Æ% vector of errors that we want to minimize with the new element  
...
[betterDictionaryElement,singularValue,betaVector] = svds(errors,1);%betterDictionaryElement‰∏∫Âè≥?πÂ??ëÈ? singularValue‰∏∫Ê?Â§ßÁâπÂæÅÂ? betaVectorÂ∑¶ÁâπÂæÅÂ??? 

CoefMatrix(j,relevantDataIndices) = singularValue*betaVector';% *signOfFirstElem Á≥ªÊï∞?©Èòµ?ÑÁ¨¨jË°åÁ??ûÈõ∂?ÉÁ?‰ΩçÁΩÆ?¢‰∏∫singularValue*betaVector?ÑÂ?  

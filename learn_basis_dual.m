...
if exist('Binit', 'var')
    ...
    %dual_lambda = diag(inv(Binit)*XSt - SSt);
    dual_lambda = diag(pinv(Binit)*XSt - SSt);
else
...
fobj_basis_dual(dual_lambda, SSt, XSt, X, c, trXXt)
...
function [f,g,H] = fobj_basis_dual(dual_lambda, SSt, XSt, X, c, trXXt)			%value of function, gradient, hessian matrix
% Compute the objective function value at x
L= size(XSt,1);
M= length(dual_lambda);

%SSt_inv = inv(SSt + diag(dual_lambda));
SSt_inv = pinv(SSt + diag(dual_lambda));
